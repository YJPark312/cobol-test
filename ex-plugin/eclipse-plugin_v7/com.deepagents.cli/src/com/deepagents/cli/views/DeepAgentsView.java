...
public class DeepAgentsView extends ViewPart {
    ...
    private Composite   confirmPanel;      // auto-approve 확인 패널
    ...
    private void buildWorkspaceContent(Composite panel) {
        ...
        MenuItem downloadFolderItem = new MenuItem(contextMenu, SWT.PUSH);
        downloadFolderItem.setText("폴더 통째로 저장");
        downloadFolderItem.addListener(SWT.Selection, e -> {
            TreeItem[] sel = workspaceTree.getSelection();
            if (sel.length == 0) return;
            TreeItem item = sel[0];
            Boolean isDir = (Boolean) item.getData("isDir");
            if (isDir == null || !isDir) {
                execJs(HtmlRenderer.warningMessage("폴더를 선택하세요."));
                return;
            }
            downloadWorkspaceFolder(item);
        });
    ...
    /** 폴더 트리 아이템의 모든 파일 경로를 재귀로 수집 */
    private void collectFolderFilePaths(TreeItem item, List<String> result) {
        for (TreeItem child : item.getItems()) {
            Boolean isDir = (Boolean) child.getData("isDir");
            if (isDir != null && isDir) {
                collectFolderFilePaths(child, result);
            } else {
                String path = (String) child.getData("path");
    ...
    /**
     * 선택한 폴더의 모든 파일을 openWorkspaceFile과 동일한 방식으로
     * Eclipse 프로젝트 c2j-workdir/ 아래에 일괄 생성한다.
     */
    private void downloadWorkspaceFolder(TreeItem folderItem) {
        if (currentSessionId == null) {
            execJs(HtmlRenderer.warningMessage("세션이 없습니다."));
            return;
        }
        List<String> paths = new ArrayList<>();
        collectFolderFilePaths(folderItem, paths);
        if (paths.isEmpty()) {
            execJs(HtmlRenderer.warningMessage("폴더 안에 파일이 없습니다."));
            return;
        }

        String folderPath = (String) folderItem.getData("path");
        String sessionId  = currentSessionId;

        execJs(HtmlRenderer.systemMessage("폴더 저장 시작: " + folderPath + " (" + paths.size() + "개 파일)"));
        ...
        new Thread(() -> {
            ...
            execJs(HtmlRenderer.systemMessage(
                "폴더 저장 완료: " + folderPath + " — 성공 " + s + "개" + (f > 0 ? ", 실패 " + f + "개" : "")));
        }, "c2j-download-folder").start();
    }
    ...
    private void initSession() {
        ...
        // 서버 허용 패턴 ^[a-zA-Z0-9_\-]{1,64}$ 에 맞게 정제
        // 도메인\계정(Windows), 공백, 특수문자 등 제거
        uid = uid.replaceAll("[^a-zA-Z0-9_\\-]", "_");  // 허용 외 문자 → _
        if (uid.isEmpty()) uid = "eclipse-user";
        ...
        new Thread(() -> {
            try {
                ...
                // workspace 트리 초기 로드
                try {
                    List<UserFileInfo> wsFiles = apiClient.listWorkspaceFiles(sessionId);
    ...
    private void loadWorkspaceFiles() {
        if (currentSessionId == null) {
            execJs(HtmlRenderer.warningMessage("Workspace 로드 실패: 세션 미연결"));
            return;
        }
        final String sid = currentSessionId;
        new Thread(() -> {
            try {
                List<UserFileInfo> files = apiClient.listWorkspaceFiles(sid);
                getSite().getShell().getDisplay().asyncExec(() -> {
    ...
    private void buildServerFileTree(List<UserFileInfo> files) {
        System.out.println("[C2J] buildServerFileTree: " + files.size() + " files");
    ...
    public void startConversion(String programName) {
        ...
        // 툴바 Auto-approve ON이면 바로 시작, 아니면 UI 내 confirm 패널 표시
        if (autoApproveCheck != null && autoApproveCheck.getSelection()) {
            doStartConversion(programName, true);
        } else {
            showConfirmPanel(programName);
        }
    }

    /** bottomArea에 auto-approve 확인 패널을 띄운다 (interrupt 패널과 동일한 방식). */
    private void showConfirmPanel(String programName) {
        if (confirmPanel != null && !confirmPanel.isDisposed()) confirmPanel.dispose();
        Display display = getSite().getShell().getDisplay();

        confirmPanel = new Composite(bottomArea, SWT.BORDER);
        confirmPanel.setLayout(new GridLayout(1, false));
        confirmPanel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        confirmPanel.setBackground(display.getSystemColor(SWT.COLOR_DARK_BLUE));

        Label infoLabel = new Label(confirmPanel, SWT.WRAP);
        infoLabel.setText("▶ C2J 전환: " + programName + "\n툴 호출 시 매번 승인하시겠습니까?");
        infoLabel.setForeground(display.getSystemColor(SWT.COLOR_WHITE));
        infoLabel.setBackground(display.getSystemColor(SWT.COLOR_DARK_BLUE));
        infoLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));

        Composite btnRow = new Composite(confirmPanel, SWT.NONE);
        btnRow.setLayout(new RowLayout(SWT.HORIZONTAL));
        btnRow.setBackground(display.getSystemColor(SWT.COLOR_DARK_BLUE));

        Button manualBtn = new Button(btnRow, SWT.PUSH);
        manualBtn.setText("수동 승인");
        manualBtn.addListener(SWT.Selection, e -> {
            dismissConfirmPanel();
            doStartConversion(programName, false);
        });

        Button autoBtn = new Button(btnRow, SWT.PUSH);
        autoBtn.setText("자동 승인");
        autoBtn.addListener(SWT.Selection, e -> {
            dismissConfirmPanel();
            doStartConversion(programName, true);
        });
        ...
        cancelBtn.addListener(SWT.Selection, e -> dismissConfirmPanel());

        bottomArea.layout(true, true);
        bottomArea.getParent().layout(true, true);
    }

    private void dismissConfirmPanel() {
        if (confirmPanel != null && !confirmPanel.isDisposed()) {
    ...
    private void doStartConversion(String programName, boolean autoApprove) {
        ...
        String sessionId = currentSessionId;
        ConversionRequest req = new ConversionRequest(programName, autoApprove);
    ...
    private SessionStreamCallback makeCallback(String sessionId) {
        return new SessionStreamCallback() {
            ...
            // onToolStart에서 count를 로컬 변수에 캡처 → onToolEnd가 정확한 chip ID 참조
            ...
            @Override public void onToolEnd(String tool) {
                execJs(HtmlRenderer.toolEnd(tool, lastToolCount));
            }

            @Override public void onInterrupt(List<ActionRequest> actions) {
                // 열린 tool-wrap이 있으면 먼저 닫기 (스피너 잔류 방지)
                if (inToolGroup) {
                    execJs(HtmlRenderer.closeToolGroup());
                    inToolGroup = false;
                }
    ...
    private void resumeWithDecision(String decision) {
        ...
        // inAssistantTurn / inToolGroup 초기화 후 새 턴 시작
        inAssistantTurn = false;
        inToolGroup     = false;
        execJs(HtmlRenderer.assistantTurnStart());
        inAssistantTurn = true;
