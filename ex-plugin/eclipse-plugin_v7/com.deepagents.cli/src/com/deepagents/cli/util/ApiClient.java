...
public class ApiClient {
    ...
    public ApiClient(String baseUrl) {
        this.fixedBaseUrl = baseUrl.endsWith("/") ? baseUrl.substring(0, baseUrl.length() - 1) : baseUrl;
    }
    ...
    public void closeSession(String sessionId) {
        try {
            URL url = new URL(getBaseUrl() + "/session/" + sessionId);
    ...
    /**
     * GET /session/{sessionId}/workspace → 세션 workdir 전체 파일 목록 조회.
     * sessionId만으로 파일 목록을 가져온다 (userId 불필요).
     */
    public List<UserFileInfo> listWorkspaceFiles(String sessionId) throws IOException {
        String json = readBody(openGet("/session/" + sessionId + "/workspace"));
        return parseUserFileList(json);
    }

    /**
     * GET /users/{userId}/files → 사용자 workdir 전체 파일 목록 조회 (레거시).
     */
    ...
    private void streamSse(String path, String jsonBody, SessionStreamCallback cb) {
        try {
            ...
            conn.disconnect();
            // done 이벤트 없이 스트림이 끊긴 경우 (nginx timeout 등) → onDone으로 마무리
            if (!doneReceived) {
                cb.onDone("end_turn", java.util.Collections.emptyList());
            }
        } catch (Exception e) {
            // Premature EOF 등 네트워크 단절 — done 없이 끊긴 경우 onDone으로 마무리
            String msg = e.getMessage();
            if (msg != null && (msg.contains("Premature EOF")
