...
public class HtmlRenderer {
    ...
    public static String connectionInfo(String sessionId, String userId, int programCount) {
        return jsAppend(
    ...
    /** 토큰 스트리밍 — current-turn(response 맨 끝)에 누적, 작업 중 인디케이터 제거 */
    public static String appendToken(String text) {
        ...
        return "if(!window._buf)window._buf='';"
             ...
             + "if(el){"
             // 첫 토큰 도착 시 작업 중 인디케이터 제거
             + "  var th=el.querySelector('.thinking');"
             + "  if(th){th.remove();}"
             + "  el.innerHTML=parseMd(window._buf);"
             + "}"
             + "scrollBottom();";
    }
    ...
    /** 툴 완료 — 스피너→체크, 전부 완료 시 자동 접기 + 작업 중 인디케이터 표시 */
    public static String toolEnd(String toolName, int count) {
        ...
        return "var chip=document.getElementById('" + chipId + "');"
             ...
             + "    updateToolCounter(tw);"
             + "    if(tw.dataset.done===tw.dataset.total){"
             + "      tw.open=false;"
             // 모든 툴 완료 → current-turn에 작업 중 인디케이터 삽입
             + "      var ct=document.getElementById('current-turn');"
             + "      if(!ct){"
             + "        var resp=document.getElementById('current-response');"
             + "        if(resp){ct=document.createElement('div');ct.id='current-turn';resp.appendChild(ct);}"
             + "      }"
             + "      if(ct&&!ct.querySelector('.thinking')){"
             + "        ct.innerHTML='<span class=\\'thinking\\'><span class=\\'thinking-dot\\'></span>"
    ...
    private static final String CSS =
        ...
        + "\n"
        + "/* ── 작업 중 인디케이터 ── */\n"
        + ".thinking { color: #8b949e; font-size: 12px; display: inline-flex; align-items: center; gap: 3px; }\n"
        + ".thinking-dot {\n"
