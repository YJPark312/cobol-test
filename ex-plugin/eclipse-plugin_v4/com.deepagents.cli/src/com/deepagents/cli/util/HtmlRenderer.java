...
/**
 * DeepAgents CLI 스타일 HTML 렌더링 엔진.
 * 터미널/CLI 느낌의 컴팩트한 출력.
...
public class HtmlRenderer {
    ...
    /** 어시스턴트 응답 턴 시작 — 하나의 response 블록 안에 텍스트+툴 공존 */
    public static String assistantTurnStart() {
        return "if(!document.getElementById('current-response')){"
             + jsAppendRaw(
                "<div class='response' id='current-response'>"
                + "<div class='resp-text' id='current-turn'></div>"
                + "</div>"
             )
             + "window._buf='';"
             + "}scrollBottom();";
    ...
    public static String toolStart(String toolName, int count) {
        ...
        return "var resp=document.getElementById('current-response');"
             ...
             + "}"
             // tool-bar가 없거나 닫혀있으면 새로 생성
             + "var tb=resp.querySelector('.tool-bar:last-child');"
             + "if(!tb||tb.dataset.closed==='1'){"
             + "  tb=document.createElement('div');tb.className='tool-bar';"
             + "  resp.appendChild(tb);"
             + "}"
    ...
    public static String closeToolGroup() {
        return "var resp=document.getElementById('current-response');"
             ...
             + "  var tb=resp.querySelector('.tool-bar:last-child');"
             + "  if(tb){tb.dataset.closed='1';}"
             + "  if(!document.getElementById('current-turn')){"
             + "    var ta=document.createElement('div');ta.className='resp-text';ta.id='current-turn';"
             + "    resp.appendChild(ta);"
             + "  }"
             + "}"
             + "window._buf='';";
    ...
    /** 완료 — 파일 수만 요약 표시 */
    public static String doneMessage(String stopReason, int fileCount) {
        String label;
        switch (stopReason) {
            case "end_turn":        label = "응답 완료"; break;
            case "cancelled":       label = "취소됨"; break;
            case "budget_exceeded": label = "예산 초과"; break;
            case "error":           label = "오류"; break;
            default:                label = stopReason; break;
        }
        String fileInfo = fileCount > 0 ? " — 생성 파일 " + fileCount + "개" : "";
        return jsAppend("<div class='done'>✅ " + esc(label) + fileInfo + "</div>");
    }
    ...
    private static final String CSS =
        ...
        + "\n"
        + "/* ── 시스템 ── */\n"
        + ".sys { color: #484f58; font-size: 12px; margin: 6px 0; }\n"
        + "\n"
        + "/* ── 사용자 입력 ── */\n"
        + ".user {\n"
