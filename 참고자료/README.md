...
| z-kesa_full.txt | z-KESA 가이드 PDF에서 추출한 전문 텍스트 |
| z-kesa_tables.txt | PDF에서 추출한 표 391개 (4,365줄). API 시그니처, 필드 정의, 코드값 테이블 등 |

### n-kesa/ (타겟 — 전환 목표)

| 파일 | 설명 |
|------|------|
| 코어뱅킹 현대화_FW_개발가이드_20260312_V0.71.docx | n-KESA 프레임워크 기반 Java 개발 가이드. 코딩 규칙, 패키지 구조, 공통 모듈 사용법 |
| n-kesa_full.txt | n-KESA 개발가이드 DOCX에서 추출한 전문 텍스트 |
| n-kesa_tables.txt | DOCX에서 추출한 표 220개 (1,601줄). API 시그니처, 필드 정의, 코드값 테이블 등 |
...
### mapping/rules/ (매핑룰 — GraphRAG 노드/엣지용)

z-KESA → n-KESA 프레임워크 전체 매핑룰을 카테고리별로 정리한 문서입니다. 상세 내용은 [rules/README.md](framework/mapping/rules/README.md) 참조.

| 파일 | 설명 |
|------|------|
| 01_architecture_structure.md | 아키텍처, 프로그램 계층, DIVISION/SECTION, 호출, CommonArea, IO편집, 연동거래, 에러처리 |
| 02_db_access.md | DBIO/SQLIO → XSQL/DBIO Unit, 커서패턴, SQL바인딩, Lock/Isolation, Array처리 |
| 03_batch_processing.md | 배치 프로그램 구조, JCL→Shell, 파일I/O, DB접근, Restart, Commit, 병렬처리 |
| 04_macros_utilities.md | 매크로 24종(#ERROR, #DYCALL 등), 유틸리티 12종(ZSGTIME, ZUGCDCV 등) |
| 05_dataset_coding_guide.md | COBOL→Java 데이터타입, MOVE/INITIALIZE/STRING, IDataSet API, BigDecimal |
| 06_eai_v3_integration.md | EAI 연계(callOutbound/sendOutbound), XIO설계, V3표준전문, MCI BID/PUSH |
...
코어뱅킹 현대화 프로젝트의 전체 아키텍처 및 인프라 표준을 정의하는 자료입니다.
