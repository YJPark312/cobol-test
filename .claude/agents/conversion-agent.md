...
| 순서 | 경로 | 목적 |
|------|------|------|
| 1 | `.claude/context/zkesa-nkesa-mapping.md` | **핵심 참조** — 프레임워크 매핑, 매크로 변환, 호출 패턴, DB 접근 패턴 |
| 2 | `.claude/context/db-meta.md` | DB 테이블 스키마, 컬럼명 |
| 3 | `output/conversion_plan.md` | 설계 결정 사항 (클래스 구조, 통합 전략, 에러코드 매핑) |
| 4 | `output/analysis_spec.md` | 원본 COBOL 로직 (비즈니스 규칙, 데이터 타입) |
| 5 | `cobol/*.cbl`, `cobol/*.cpy` | 원본 COBOL 소스 + Copybook 직접 참조 |

### 직접 전환 모드

| 순서 | 경로 | 목적 |
|------|------|------|
| 1 | `.claude/context/zkesa-nkesa-mapping.md` | **핵심 참조** — 모든 변환 판단의 기준 |
| 2 | `.claude/context/db-meta.md` | DB 테이블 스키마, 컬럼명 |
| 3 | `cobol/*.cbl`, `cobol/*.cpy` | 원본 COBOL 소스 + Copybook 직접 참조 |
| 4 | `output/analysis_spec.md` (있으면) | 원본 COBOL 로직 보충 참조 |

> ⚠ **zkesa-nkesa-mapping.md는 어느 모드든 반드시 1순위로 읽는다.** 이 파일이 모든 변환 판단의 기준이다.
