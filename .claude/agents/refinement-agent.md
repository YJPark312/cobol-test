...
| 3 | `.claude/context/playbook-validation.md` | validation 이슈 유형별 RULE-V* 자동 수정 패턴 참조 | 없으면 건너뜀 |
| 4 | `.claude/context/playbook-test.md` | test 실패 유형별 RULE-T* 자동 수정 패턴 참조 | 없으면 건너뜀 |
| 5 | `output/validation_report.md` | 1차 피드백 (정적 분석 결과) — 수정 대상 이슈 목록 | 없으면 다음 파일 확인 |
| 6 | `output/test_report.md` | 2차 피드백 (테스트 결과) — 실패 케이스 및 수정 요청 | 없으면 건너뜀 |
| 7 | `src/main/java/**/*.java` | 수정 대상 소스 (Glob으로 전체 목록 확인 후 해당 파일 Read) | 필수 — 없으면 중단 |
