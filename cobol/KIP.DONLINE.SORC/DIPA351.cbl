      ...
      *-----------------------------------------------------------------
       01  CO-AREA.
           03  CO-PGM-ID               PIC  X(008) VALUE 'DIPA351'.
      ...
      *@   처리구분 체크
           IF XDIPA351-I-PRCSS-DSTCD = SPACE
      *       필수항목 오류입니다.
      *       처리구분코드를 입력해 주십시오.
      ...
      *@   기업집단그룹코드 체크
           IF XDIPA351-I-CORP-CLCT-GROUP-CD = SPACE
      *       필수항목 오류입니다.
      ...
      *@   기업집단등록코드 체크
           IF XDIPA351-I-CORP-CLCT-REGI-CD = SPACE
      *       필수항목 오류입니다.
      ...
      *@   평가기준년 체크
           IF XDIPA351-I-VALUA-BASE-YR = SPACE
      *       기준년도 오류입니다.
      *       입력값을 확인하시기 바랍니다.
      ...
      *    '03' : 등록
      *    '04' : 수정
           EVALUATE XDIPA351-I-PRCSS-DSTCD
               WHEN '01'
      *             관계기업군재무생성계열기업 조회
      ...
      *             관계기업군재무생성계열기업 등록
                    ...
                    VARYING WK-I  FROM 1  BY 1
                      UNTIL WK-I >  XDIPA351-I-TOTAL-NOITM
      ...
      *    기업집단그룹코드
           MOVE XDIPA351-I-CORP-CLCT-GROUP-CD
             TO XQIPA181-I-CORP-CLCT-GROUP-CD
      *    기업집단등록코드
           MOVE XDIPA351-I-CORP-CLCT-REGI-CD
      ...
      *    총건수
           MOVE  DBSQL-SELECT-CNT
             TO  XDIPA351-O-TOTAL-NOITM

      *    현재건수
           IF  DBSQL-SELECT-CNT  >  CO-MAX-100  THEN
               MOVE  CO-MAX-100
                 TO  XDIPA351-O-PRSNT-NOITM
           ELSE
               MOVE  DBSQL-SELECT-CNT
                 TO  XDIPA351-O-PRSNT-NOITM
           END-IF
           ...
           PERFORM VARYING WK-I FROM 1 BY 1
                   UNTIL WK-I >  XDIPA351-O-PRSNT-NOITM

      *            고객식별자
                   MOVE XQIPA181-O-CUST-IDNFR(WK-I)
                     TO XDIPA351-O-CUST-IDNFR(WK-I)
      *            업체명
                   MOVE XQIPA181-O-ENTP-NAME(WK-I)
                     TO XDIPA351-O-ENTP-NAME(WK-I)
      ...
      *-----------------------------------------------------------------
       S3110-YMD-CALC-RTN.
           ...
           #USRLOG "★[S3110-YMD-CALC-RTN]"

      *    전년 = 기준년 - 1
           MOVE XDIPA351-I-VALUA-BASE-YR
             TO WK-PYY
      ...
      *            평가대상여부1
                   IF  XQIPA182-O-VALUA-TAGET-YN1(WK-I) = CO-TAGET-ZERO
                       MOVE CO-TAGET-N
                         TO XDIPA351-O-VALUA-TAGET-YN1(WK-I)
                   ELSE
                       MOVE CO-TAGET-Y
                         TO XDIPA351-O-VALUA-TAGET-YN1(WK-I)
                   END-IF
      ...
      *            평가대상여부2
                   IF  XQIPA182-O-VALUA-TAGET-YN2(WK-I) = CO-TAGET-ZERO
                       MOVE CO-TAGET-N
                         TO XDIPA351-O-VALUA-TAGET-YN2(WK-I)
                   ELSE
                       MOVE CO-TAGET-Y
                         TO XDIPA351-O-VALUA-TAGET-YN2(WK-I)
                   END-IF
      ...
      *            평가대상여부3
                   IF  XQIPA182-O-VALUA-TAGET-YN3(WK-I) = CO-TAGET-ZERO
                       MOVE CO-TAGET-N
                         TO XDIPA351-O-VALUA-TAGET-YN3(WK-I)
                   ELSE
                       MOVE CO-TAGET-Y
                         TO XDIPA351-O-VALUA-TAGET-YN3(WK-I)
                   END-IF
      ...
      *    일자 계산
           PERFORM S3110-YMD-CALC-RTN
              THRU S3110-YMD-CALC-EXT
      ...
      *@   TKIPA130-PK & TKIPA130-REC SET
      *    고객식별자
           MOVE XDIPA351-I-CUST-IDNFR(WK-I)
             TO KIPA130-PK-CUST-IDNFR
                RIPA130-CUST-IDNFR
      ...
      *              데이터를 생성할 수 없습니다.
      *              업무 담당자에게 문의해 주세요.
                     MOVE   '09'            TO   XDIPA351-R-STAT
                     MOVE    DBIO-SQLCODE   TO   XDIPA351-R-SQL-CD
      ...
      *    등록년월일
           MOVE XDIPA351-I-REGI-YMD2(WK-I)
      ...
      *    평가대상여부1
           IF  XDIPA351-I-VALUA-TAGET-YN1(WK-I) = CO-TAGET-Y
               MOVE CO-TAGET-ONE
      ...
      *    평가대상여부2
           IF  XDIPA351-I-VALUA-TAGET-YN2(WK-I) = CO-TAGET-Y
               MOVE CO-TAGET-ONE
      ...
      *    평가대상여부3
           IF  XDIPA351-I-VALUA-TAGET-YN3(WK-I) = CO-TAGET-Y
               MOVE CO-TAGET-ONE
           ...
           #USRLOG "★[대상여부1]=" RIPA130-VALUA-TAGET-YN1
           #USRLOG "★[대상여부2]=" RIPA130-VALUA-TAGET-YN2
           #USRLOG "★[대상여부3]=" RIPA130-VALUA-TAGET-YN3
      ...
      *    업체명
           MOVE XDIPA351-I-ENTP-NAME(WK-I)
      ...
      *    등록부점코드
           MOVE XDIPA351-I-REGI-BRNCD
             TO RIPA130-REGI-BRNCD

      *    등록직원번호
           MOVE XDIPA351-I-REGI-EMPID
