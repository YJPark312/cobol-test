      ...
      *-----------------------------------------------------------------
       01  CO-AREA.
           03  CO-PGM-ID               PIC  X(008) VALUE 'BIP0022'.
           03  CO-STAT-OK              PIC  X(002) VALUE '00'.
           03  CO-STAT-ERROR           PIC  X(002) VALUE '09'.
           03  CO-STAT-ABNORMAL        PIC  X(002) VALUE '98'.
           03  CO-STAT-SYSERROR        PIC  X(002) VALUE '99'.
           03  CO-4                    PIC  9(002) VALUE 4.
...
161108*@  재무분석자료번호(구분+고객식별자)
      ...
      *@  계산값
           03  WK-FNAF-ANLS-ITEM-AMT   PIC S9(015) COMP-3.
      ...
      *@  결산년합계업체수
           03  WK-STLACC-CNT           PIC  S9(005) COMP-3.
      ...
      *-----------------------------------------------------------------
           EXEC SQL DECLARE CUR_C003 CURSOR FOR
                ...
                SELECT Z."재무분석보고서구분"
                   ...
                   ,CASE WHEN (LENGTH(REPLACE(Z."계산식내용",' ',''))
                             - LENGTH(REPLACE
                                     ...
                                     ,' ',''))) > 0
                   THEN
                   CASE WHEN SUBSTR(REPLACE(Z."계산식내용",' ',''),
               LOCATE_IN_STRING(REPLACE(Z."계산식내용",' ',''),'&',1,
                      1,OCTETS) + 1,8) > 'A'
                      THEN
               ...
               ,LOCATE_IN_STRING(REPLACE(Z."계산식내용",' ',''),'&'
                         ,1,1,OCTETS) + 1,2)
                         BETWEEN '11' AND '17' THEN
                       (SELECT  CHAR(
                              C."재무제표항목금액"
                              ) "값"
                          FROM DB2DBA.THKIPC120 C
                         WHERE C."그룹회사코드"
                             = :BICOM-GROUP-CO-CD
                           AND C.기업집단그룹코드
               ...
               ,
                CASE WHEN SUBSTR(REPLACE(Z."계산식내용",' ','')
               ,LOCATE_IN_STRING(REPLACE(Z."계산식내용",' ',''),'&'
                          ,1,1,OCTETS) + 1,2) = '18' THEN
                ('#' || REPLACE(SUBSTR(REPLACE(Z."계산식내용",' ','')
               ,LOCATE_IN_STRING(REPLACE(Z."계산식내용",' ',''),'&'
                                  ,1,1,OCTETS) + 1,8),'-','_') || '#'
                        ) END
                        ,
               ...
               UNION ALL
                  ...
                  , MA11_KEY
                  , SEQNO + 1
                  , CASE WHEN (LENGTH(A."계산식내용")
                            - LENGTH(REPLACE(A."계산식내용",'#','')
                              )) > 0
                         THEN '0'
                         ELSE '1'
                         END END_CLASF
                  ...
                  , MA11_KEY
                  , SEQNO + 1
                  , CASE WHEN B.END_CLASF = '0' AND
                             (LENGTH(B."계산식내용")
                            - LENGTH(REPLACE(B."계산식내용",'#','')
                              )) > 0
                         THEN '0'
                         ELSE '1'
                         END END_CLASF
                  ,CASE WHEN B.END_CLASF = '0' AND
                        ...
                        THEN OVERLAY(B."계산식내용"
                             ,'(' ||
                                  (SELECT X."계산식내용"
                             FROM MA11A X
                            WHERE X.MA11_KEY
                                = SUBSTR(B."계산식내용"
                                 ,LOCATE_IN_STRING(B."계산식내용"
                                 ,'#',1,1
                                 ,OCTETS
                                 ) + 1,7
                                 )
      ...
      *       결산년
               PERFORM S3220-STLACC-YR-SELECT-RTN
                  THRU S3220-STLACC-YR-SELECT-EXT
       ...
       S3200-THKIPC110-FETCH-EXT.
           EXIT.
      *-----------------------------------------------------------------
      *@  기존 결산년 합산현금수지 존재여부확인
      *-----------------------------------------------------------------
       S3210-EXIST-C120-DATA-RTN.

           MOVE  'C120 DELETE STEP'    TO XZUGDBUD-I-FUNCTION.
           MOVE  'THKIPC120 DEL.' TO XZUGDBUD-I-DESC.
           #DYCALL ZUGDBUD YCCOMMON-CA  XZUGDBUD-CA
           IF    NOT COND-XZUGDBUD-OK
      ...
      *   존재하는게 있다면 삭제 후 재생성
            ...
            PERFORM  S3211-EXIST-C120-DATA-RTN
               THRU  S3211-EXIST-C120-DATA-EXT
      ...
      *@  합산현금수지 생성로직
           ...
           PERFORM S3221-STLACC-YR-SELECT-RTN
              THRU S3221-STLACC-YR-SELECT-EXT
       ...
       S3220-STLACC-YR-SELECT-EXT.
           EXIT.
      *-----------------------------------------------------------------
      *@  결산년
      *-----------------------------------------------------------------
       S3221-STLACC-YR-SELECT-RTN.
      ...
      *   결산년합계업체수
           ...
           MOVE  WK-STLACC-YR-CH
             TO  RIPC120-STLACC-YR

           MOVE  WK-DB-FNAF-A-RPTDOC-DSTIC
