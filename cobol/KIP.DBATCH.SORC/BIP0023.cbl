      ...
      *-----------------------------------------------------------------
       01  CO-AREA.
           03  CO-PGM-ID               PIC  X(008) VALUE 'BIP0023'.
      ...
      *@  변경테이블ID
           03  CO-TABLE-NM             PIC  X(010) VALUE 'THKIPC121'.
...
161108*@  재무분석자료번호(구분+고객식별자)
      ...
      *       기존 기준년 합산재무비율 삭제
               PERFORM S3210-EXIST-C121-DATA-RTN
                  THRU S3210-EXIST-C121-DATA-EXT
      ...
      *-----------------------------------------------------------------
       S3210-EXIST-C121-DATA-RTN.
           ...
           MOVE  'C121 DELETE STEP'    TO XZUGDBUD-I-FUNCTION.
           MOVE  'THKIPC121 DEL.' TO XZUGDBUD-I-DESC.
           #DYCALL ZUGDBUD YCCOMMON-CA  XZUGDBUD-CA
           IF    NOT COND-XZUGDBUD-OK
                 DISPLAY 'C121 DELETE - 파일기록 에러입니다'
           END-IF
      ...
      *   존재하는게 있다면 삭제 후 재생성
            ...
            PERFORM  S3211-EXIST-C121-DATA-RTN
               THRU  S3211-EXIST-C121-DATA-EXT
      ...
      *   COMMIT 처리 : WK-COMMIT-POINT 단위로 COMMIT.
       ...
       S3210-EXIST-C121-DATA-EXT.
           EXIT.
      *-----------------------------------------------------------------
      *@  기존 합산 재무비율 목록 삭제
      *-----------------------------------------------------------------
       S3211-EXIST-C121-DATA-RTN.
      ...
      *@1  SQLIO 호출
           EXEC SQL
                FETCH  CUR_C004
                INTO
                     :KIPC121-PK-GROUP-CO-CD
                   , :KIPC121-PK-CORP-CLCT-GROUP-CD
                   , :KIPC121-PK-CORP-CLCT-REGI-CD
                   , :KIPC121-PK-FNAF-A-STLACC-DSTCD
                   , :KIPC121-PK-BASE-YR
                   , :KIPC121-PK-STLACC-YR
                   , :KIPC121-PK-FNAF-A-RPTDOC-DSTCD
                   , :KIPC121-PK-FNAF-ITEM-CD
           END-EXEC.
      ...
      *@1       DBIO 호출
                #DYDBIO SELECT-CMD-Y TKIPC121-PK TRIPC121-REC

      *         DBIO 호출결과 확인
                IF NOT COND-DBIO-OK   AND
                   NOT COND-DBIO-MRNF
                    DISPLAY 'C121 SELECT 에러입니다'
      *@1           처리종료
                     PERFORM S9000-FINAL-RTN
                        THRU S9000-FINAL-EXT
                END-IF
      ...
      *@1           처리종료
           ...
           WHEN OTHER
                ...
                DISPLAY "FETCH  CUR_C004 "
                        ...
                        "  SQLSTATE:[" SQLSTATE "]"
                        "   SQLERRM:[" SQLERRM  "]"
                MOVE 'THKIPC121'     TO XZUGEROR-I-TBL-ID
                MOVE 'FETCH'         TO XZUGEROR-I-FUNC-CD
                MOVE SQLCODE         TO XZUGEROR-I-SQL-CD
                MOVE 'FETCH ERROR'   TO XZUGEROR-I-MSG
      *@1      사용자정의 에러코드 설정(22: FETCH 오류)
