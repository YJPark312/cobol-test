...
220128*김경호:20220128:인스턴스오류분 변경(법인그룹연결등록구분)
      * ----------------------------------------------------------------
240405*김경호:한신평 그룹제외정보(그룹코드없을경우) 반영
      *         전산등록분일 경우 한신평 그룹코드가 없을경우
      *         그룹정보초기화 및 등록내역 초기화
      *=================================================================
      ...
      *    일련번호
           ...
           03  WK-SKIP-CNT1             PIC  9(005).
           03  WK-SKIP-CNT2             PIC  9(005).
...
240227*    입력레코드
      ...
      *-----------------------------------------------------------------
240405**   한신평업체개요 변동분처리시 대표사업자정보 연계추가
      *-----------------------------------------------------------------
           EXEC SQL
              DECLARE CUR_MAIN CURSOR WITH HOLD FOR
              SELECT CB01.한신평업체코드
              ...
              AND   CB01.최종변경년월일 =
                         (SELECT MAX(최종변경년월일)
                          ...
                          WHERE  그룹회사코드 = CB01.그룹회사코드
                          AND    고객식별자   = CB01.고객식별자)
           END-EXEC.
      *=================================================================
       PROCEDURE                       DIVISION.
      *=================================================================
...
240405*    변경로그처리
           IF  WK-INSERT-CNT = 100
           OR  WK-UPDATE-CNT = 100
           THEN
      *        ------------------------------
      *        BATCH CHANG LOG 기록 종료
      *        ------------------------------
      *       로그끝 #############################################
               SET COND-DBIO-CHGLOG-NO  TO  TRUE
...
220128*        법인그룹연결등록구분(1.전산)
      ...
      *@1  호출 결과 검증
           ...
           WHEN OTHER
                ...
                DISPLAY "THKIPA110 INSERT ERR"
                        " CUST-IDNFR = " KIPA110-PK-EXMTN-CUST-IDNFR
                        " SQL-ERROR = " WK-SQLCODE
                        " SQLSTATE  = " SQLSTATE
      ...
      *     . 등록직원명
           ...
           EVALUATE SQLCODE
           WHEN ZERO
                DISPLAY "A112 INSERT : " KIPA112-PK-EXMTN-CUST-IDNFR
                CONTINUE
           WHEN OTHER
                MOVE SQLCODE TO WK-SQLCODE
                DISPLAY "THKIPA112 INSERT ERR"
                        " CUST-IDNFR = " KIPA112-PK-EXMTN-CUST-IDNFR
                        " SQL-ERROR = " WK-SQLCODE
                        " SQLSTATE  = " SQLSTATE
           END-EVALUATE
           .
...
170210*    기업군관리그룹구분
      ...
      *@1  호출 결과 검증
           EVALUATE TRUE
           WHEN COND-DBIO-OK
                DISPLAY "PA111 INSERT : "
                         KIPA110-PK-EXMTN-CUST-IDNFR

           WHEN OTHER
                MOVE SQLCODE TO WK-SQLCODE
                DISPLAY "THKIPA111 INSERT ERR"
                        " CUST-IDNFR = " KIPA110-PK-EXMTN-CUST-IDNFR
                        " SQL-ERROR = " WK-SQLCODE
                        " SQLSTATE  = " SQLSTATE
      ...
      *          DISPLAY "A111 UPDATE : "
      *                   KIPA111-PK-CORP-CLCT-GROUP-CD  '-'
      *                   KIPA111-PK-CORP-CLCT-REGI-CD

                CONTINUE

           WHEN OTHER
                MOVE SQLCODE TO WK-SQLCODE
                DISPLAY "THKIPA111 UPDATE ERR"
                        " CUST-IDNFR = " KIPA110-PK-EXMTN-CUST-IDNFR
                        " SQL-ERROR = " WK-SQLCODE
                        " SQLSTATE  = " SQLSTATE
           END-EVALUATE
      ...
      *@1 작업결과　메세지　처리
           ...
           DISPLAY '  전산등록변경 = ' WK-UPDATE-CNT
           DISPLAY '  신규생성건수 = ' WK-INSERT-CNT
           DISPLAY 'CRM미등록분SKIP= ' WK-SKIP-CNT1
           DISPLAY '수기등록분SKIP = ' WK-SKIP-CNT2
           DISPLAY '수기중복건수   = ' WK-HWRT-MODFI-CNT
           DISPLAY '*------------------------------------------*'
